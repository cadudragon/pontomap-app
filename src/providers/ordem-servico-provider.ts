...
/*
  Generated class for the OrdemServicoProvider provider.
...
export class OrdemServicoProvider {
  ...
  getOrdens(url){
    var credentials =  "?id_login=" + this.usuarioLogado.idLogin +""
    var headers = new Headers();
    headers.append('Content-Type', 'application/x-www-form-urlencoded');
    headers.append('Authorization', 'bearer ' + this.usuarioLogado.token );

     return this.http.get(url+'/api/os/getOrdensServico' + credentials, { headers: headers }).map(res => res.json());
  }
